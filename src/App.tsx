...
function pointLineDistanceSigned(p: Pt, a: Pt, b: Pt) {
  ...
  if (den === 0) return NaN;
  return num / den; // signed px
}

function toFixedOrDash(n: number | undefined | null, digits = 2) {
  if (n == null || Number.isNaN(n)) return "—";
  return n.toFixed(digits);
}

function zScore(value: number, mean: number, sd: number) {
  if (Number.isNaN(value) || sd <= 0) return NaN;
...
function CephTracer() {
  ...
  const [pDoctor, setPDoctor] = useState<string>(""); // vacío por defecto
  ...
  // Descarga manual (si el navegador bloquea el click programático en el sandbox)
  ...
  useEffect(() => {
    return () => {
      ...
      if (downloadHint?.url?.startsWith("blob:")) URL.revokeObjectURL(downloadHint.url);
    };
  }, [downloadHint]);
  ...
  useEffect(() => {
    try {
      console.assert(Math.abs(distance({ x: 0, y: 0 }, { x: 3, y: 4 }) - 5) < 1e-6, "distance 3-4-5");
      const right90 = angleBetween({ x: 0, y: 0 }, { x: 1, y: 0 }, { x: 1, y: 1 });
      console.assert(Math.abs(right90 - 90) < 1e-6, "angleBetween 90°");
      const lineAng = angleBetweenLines({ x: 0, y: 0 }, { x: 1, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 1 });
      console.assert(Math.abs(lineAng - 90) < 1e-6, "angleBetweenLines 90°");
      console.assert(zScore(82, 82, 3) === 0, "zScore mean");
      console.assert(Math.abs(zScore(85, 82, 3) - 1) < 1e-6, "zScore +1sd");
      const csvCheck = [["a","b"],["1","2"]].map(r=>r.join(",")).join("\r\n");
      console.assert(csvCheck.includes("\r\n"), "csv join CRLF");
      // extra tests
      const s = pointLineDistanceSigned({x:1,y:1},{x:0,y:0},{x:2,y:0});
      console.assert(!Number.isNaN(s) && Math.sign(s) === 1, "signed distance above +");
      const a180 = angleBetweenLines({x:0,y:0},{x:1,y:0},{x:1,y:0},{x:2,y:0});
      console.assert(Math.abs(a180) < 1e-6, "angleBetweenLines 0° colinear");
    } catch {}
  }, []);
  ...
  function onCanvasClick(e: React.MouseEvent) {
    ...
    if (calibMode) {
      const next = [...calibClicks, pt].slice(-2);
      setCalibClicks(next);
      if (next.length === 2) {
        const px = distance(next[0], next[1]);
        if (mmKnown > 0 && px > 0) {
          setMmPerPx(mmKnown / px);
          setCalibMode(false);
        }
      }
      return;
  ...
  function onPointMouseDown(k: LandmarkKey, e: React.MouseEvent) {
    ...
    const y = e.clientY - rect.top;
    const p = points[k]!;
    dragInfo.current = { key: k, offset: { x: x - p.x, y: y - p.y } };
  ...
  function onMove(e: MouseEvent) {
    ...
    const k = dragInfo.current.key!;

    if (rafId.current) return;
    rafId.current = requestAnimationFrame(() => {
      setPoints((prev) => ({ ...prev, [k]: { x, y } }));
      rafId.current = null;
    });
  }
  ...
  const scaleLabel = mmPerPx ? `Escala: ${(1 / mmPerPx).toFixed(2)} px/mm · ${mmPerPx.toFixed(4)} mm/px` : "Sin calibrar";

  function setManualLink(url: string, name: string) {
    if (downloadHint?.url && downloadHint.url.startsWith("blob:")) URL.revokeObjectURL(downloadHint.url);
    setDownloadHint({ url, name });
  }

  function exportJSON() {
    const blob = new Blob([JSON.stringify({ points, mmPerPx }, null, 2)], { type: "application/json" });
    triggerDownload(blob, "cefalo_trazado.json");
  }
  ...
  function importJSON(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        if (typeof data.mmPerPx === "number") setMmPerPx(data.mmPerPx);
      } catch {
        alert("JSON inválido");
      }
    };
  ...
  function exportCSV() {
    ...
    if (useSteiner) {
      rows.push(["— Steiner —", "", "", ""],
        ...
        ["L1–NB (°)", toFixedOrDash(L1_NB_deg), "°", toFixedOrDash(zScore(L1_NB_deg, DEFAULT_NORMS.steiner.L1_NB_deg.mean, DEFAULT_NORMS.steiner.SN_GoGn.sd))],
    ...
    triggerDownload(blob, "cefalo_resultados.csv");
  }
  ...
  function triggerDownload(blob: Blob, filename: string) {
    ...
    setManualLink(url, filename);
  }

  // ===== Render de lámina en canvas (reutilizable) =====
  async function renderSheetCanvas(): Promise<HTMLCanvasElement | null> {
    ...
    if (!imgW || !imgH) { alert("La imagen aún no está lista para exportar."); return null; }

    const sidebarW = 380; const pad = 24;
    const W = imgW + sidebarW + pad * 2; const H = imgH + pad * 2;
    ...
    function drawLine(a?: Pt, b?: Pt, style = "#38bdf8", dash = false) {
      if (!a || !b) return; ctx.save(); if (dash) ctx.setLineDash([6,4]);
      ctx.strokeStyle = style; ctx.lineWidth = 2; ctx.beginPath();
      ctx.moveTo(pad + a.x, pad + a.y); ctx.lineTo(pad + b.x, pad + b.y); ctx.stroke(); ctx.restore();
    }
    function drawPoint(p?: Pt) {
      if (!p) return; ctx.save(); ctx.fillStyle = "#94a3b8"; ctx.strokeStyle = "#0f172a"; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(pad + p.x, pad + p.y, 5, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); ctx.restore();
    }
    function drawArc(v?: Pt, p1?: Pt, p2?: Pt, color = "#22c55e") {
      if (!v || !p1 || !p2 || !showOverlay) return; ctx.save(); const r = 35;
      ...
      ctx.strokeStyle = color; ctx.lineWidth = 2; ctx.setLineDash([4,3]); ctx.beginPath();
      ctx.arc(pad + v.x, pad + v.y, r, a1, a2, da < 0); ctx.stroke(); ctx.restore();
    }

    // Dibujo de guías
    drawLine(points.S, points.N, "#38bdf8");
    drawLine(points.N, points.A, "#22c55e");
    drawLine(points.N, points.B, "#f97316");
    drawLine(points.Po, points.Or, "#a78bfa", true);
    drawLine(points.Go, points.Me, "#f472b6", true);
    drawLine(points.Go, points.Gn, "#94a3b8");
    if (U1_axis) drawLine(U1_axis[0], U1_axis[1], "#eab308");
    if (L1_axis) drawLine(L1_axis[0], L1_axis[1], "#22d3ee");

    Object.values(points).forEach((p) => drawPoint(p));

    drawArc(points.N, points.S, points.A, "#22c55e");
    drawArc(points.N, points.S, points.B, "#f97316");
    drawArc(points.N, points.A, points.B, "#38bdf8");
    drawArc(points.S, points.N, points.Ar, "#16a34a");
    drawArc(points.Ar, points.S, points.Go, "#d946ef");
    drawArc(points.Go, points.Ar, points.Me, "#fb7185");

    // Sidebar
    const x0 = pad + imgW + 16; let y = pad + 8;
    ctx.fillStyle = "#e2e8f0"; ctx.font = "bold 20px system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif";
    ctx.fillText("Cefalometría", x0, y); y += 22;
    ctx.font = "12px system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif"; ctx.fillStyle = "#93c5fd";
    ctx.fillText("by @dr.juarez", x0, y); y += 6;
    ctx.fillStyle = "#94a3b8"; ctx.fillText(scaleLabel, x0, y + 18); y += 36;

    function lineKV(k: string, v: string, z?: number) {
      ctx.fillStyle = "#e2e8f0"; ctx.fillText(k, x0, y);
      const vStr = v + (z == null || Number.isNaN(z) ? "" : `  (z ${z >= 0 ? "+" : ""}${z.toFixed(2)})`);
      ctx.textAlign = "right"; ctx.fillText(vStr, x0 + sidebarW - 24, y); ctx.textAlign = "left"; y += 16;
    }

    // Datos del paciente
    ctx.fillStyle = "#93c5fd"; ctx.font = "bold 14px system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif";
    ctx.fillText("Datos del paciente", x0, y); y += 18;
    ...
    if (useSteiner) {
      ctx.fillStyle = "#93c5fd"; ctx.font = "bold 14px system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif";
      ctx.fillText("Steiner", x0, y); y += 18; ctx.font = "12px system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif";
      ...
      lineKV("L1–NB (°)", toFixedOrDash(L1_NB_deg), zScore(L1_NB_deg, DEFAULT_NORMS.steiner.L1_NB_deg.mean, DEFAULT_NORMS.steiner.SN_GoGn.sd));
      ...
      lineKV("Pg–NB (mm)", toFixedOrDash(Pg_NB_mm), zScore(Pg_NB_mm, DEFAULT_NORMS.steiner.Pg_NB_mm.mean, DEFAULT_NORMS.steiner.Pg_NB_mm.sd));
      y += 6;
    }
    ...
    if (useBjork) {
      ctx.fillStyle = "#93c5fd"; ctx.font = "bold 14px system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif";
      ctx.fillText("Björk–Jarabak", x0, y); y += 18; ctx.font = "12px system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif";
  ...
  async function exportSheetPNG() {
    ...
    if (!blob) {
      ...
      setManualLink(dataUrl, "cefalometria.png");
      try {
        const a = document.createElement("a"); a.href = dataUrl; a.download = "cefalometria.png"; a.rel = "noopener"; a.target = "_blank"; document.body.appendChild(a); a.click(); a.remove();
      } catch {}
      return;
    }
    triggerDownload(blob, "cefalometria.png");
  }
...
<title>Cefalometría – PDF</title>
<style>
  @page { size: A4; margin: 16mm; }
  body{font-family: system-ui,-apple-system,Segoe UI,Roboto,Inter,Arial,sans-serif;color:#0b1220;margin:0;}
  .hdr{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:8px;}
  .brand{font-size:12px;color:#2563eb}
  .meta{font-size:12px;color:#334155;margin-bottom:12px;}
  .img{width:100%;max-width:100%;}
  a{color:#2563eb;text-decoration:underline;}
</style></head><body>
  <div class=\\"hdr\\"><h1 style=\\"margin:0;font-size:20px\\">Cefalometría</h1><div class=\\"brand\\"><a href=\\"https://www.instagram.com/dr.juarez\\" target=\\"_blank\\" rel=\\"noopener\\">by @dr.juarez</a></div></div>
  <div class=\\"meta\\">
    <div><strong>Paciente:</strong> ${pNombre || "—"}</div>
    <div><strong>Edad:</strong> ${pEdad ? pEdad+" años" : "—"} &nbsp; <strong>Sexo:</strong> ${pSexo || "—"}</div>
    <div><strong>Fecha:</strong> ${pFecha || "—"} &nbsp; <strong>Doctor:</strong> ${pDoctor || "—"}</div>
  </div>
  <img class=\\"img\\" src=\\"${dataUrl}\\" alt=\\"Lámina cefalométrica\\"/>
  <script>window.onload=()=>{setTimeout(()=>window.print(),400)}</script>
    ...
    if (!w) {
      setManualLink(url, "cefalometria.pdf.html");
    }
  }
      ...
      <div className="lg:col-span-4 space-y-4">
        <section className="rounded-2xl border border-slate-800 p-4 bg-slate-900/50">
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            ...
            <label className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-sm cursor-pointer">
              Importar JSON
        ...
        <section className="rounded-2xl border border-slate-800 p-4 bg-slate-900/50">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => { setCalibMode(true); setCalibClicks([]); }} className={`px-3 py-1.5 rounded-xl text-sm ${calibMode ? "bg-emerald-600" : "bg-slate-800 hover:bg-slate-700"}`}>
              {calibMode ? "Calibrando… (haz dos clics)" : "Iniciar calibración"}
            </button>
            <label className="text-xs text-slate-400">Longitud real (mm)</label>
            <input type="number" value={mmKnown} min={0.1} step={0.1} onChange={(e) => setMmKnown(Number(e.target.value))}
              className="w-24 bg-slate-800 border border-slate-700 rounded-lg px-2 py-1 text-sm" />
          </div>
          <div className="text-xs text-slate-400 mt-2">{scaleLabel}</div>
          {calibClicks.length === 2 && (
            <div className="text-xs text-slate-400 mt-1">Distancia de referencia: {toFixedOrDash(distance(calibClicks[0], calibClicks[1]))} px</div>
          )}
        </section>
        ...
        <section className="rounded-2xl border border-slate-800 p-4 bg-slate-900/50">
          ...
          <div className="flex items-center gap-2 mb-2">
            <label className="text-xs text-slate-400">Modo de colocación</label>
            <input type="checkbox" checked={placingMode} onChange={(e) => setPlacingMode(e.target.checked)} />
            <span className="text-xs text-slate-400">(Click para colocar / arrastrar para ajustar)</span>
          </div>
          <ul className="space-y-1 max-h-64 overflow-auto pr-1">
            {LANDMARKS.map((lm) => {
              const selected = activeKey === lm.key; const isSet = Boolean(points[lm.key]);
              return (
        ...
        <section className="rounded-2xl border border-slate-800 p-4 bg-slate-900/50">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-slate-300"><th className="text-left font-medium py-1 pr-4">Medida</th><th className="text-right font-medium py-1 pr-4">Valor</th><th className="text-left font-medium py-1">Unid</th><th className="text-left font-medium py-1">z</th></tr>
              </thead>
          ...
          {downloadHint && (
            <div className="mt-3 rounded-xl border border-slate-800 bg-slate-900/60 p-3 text-xs text-slate-300">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {lastCSV && (
                  <button onClick={async () => { try { await navigator.clipboard.writeText(lastCSV); alert("CSV copiado al portapapeles"); } catch { alert("No se pudo copiar. Abre el enlace y guarda el archivo."); } }} className="px-2 py-1 rounded-lg bg-slate-800 hover:bg-slate-700">Copiar CSV</button>
                )}
              ...
              <div className="relative inline-block" onClick={onCanvasClick}>
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  {LANDMARKS.map(({ key, label }) => {
                    const p = points[key]; if (!p) return null; const isActive = activeKey === key;
                    return (
                  ...
                  {calibMode && calibClicks.length > 0 && (
                    <g>
                  ...
                  {showOverlay && has("N") && has("S") && has("Ar") && (
                    <g>
                  ...
                  {showOverlay && has("S") && has("Ar") && has("Go") && (
                    <g>
                  ...
                  {showOverlay && has("Ar") && has("Go") && has("Me") && (
                    <g>
...
function RowZ({ name, value, units, norm }: { name: string; value: number; units: string; norm: { mean: number; sd: number } }) {
  ...
  const zz = ok ? zScore(value, norm.mean, norm.sd) : NaN;
...
function AngleLabel({ p, text }: { p: Pt; text: string }) {
  return (
    <text x={p.x} y={p.y} fontSize={12} fill="#e2e8f0" stroke="#0f172a" strokeWidth={0.5}>{text}</text>
  );
}
